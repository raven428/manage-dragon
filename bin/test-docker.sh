...
[[ "${CONTENGI}" == 'podman' ]] && {
  ...
  ANSIBLE_CONT_ADDONS='--network=host --privileged'
}
...
/usr/bin/env "${CONTENGI}" exec -t "${CONT_NAME}" sh -c 'apt-get update && apt-get \
install -y --no-install-recommends git'
[[ "${CONTENGI}" == 'podman' ]] && {
  /usr/bin/env "${CONTENGI}" exec -t "${CONT_NAME}" sh -c '\
apt-get install -y --no-install-recommends nftables fuse-overlayfs
